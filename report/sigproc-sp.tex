...
Drugi princip, ki ga uporablja CTPH, je zgoščevalna funkcija z drsečim oknom (angl. \emph{rolling hash}) preslika zadnjih $k$ zlogov (bajtov) v psevdonaključno vrednost. Vsakega naslednika je možno hitro izračunati iz predhodno izračunane vrednosti.
...
Nazadnje algoritem primerja prstne odtise datotek, torej zaporedje Bloomovih filtrov. Izračuna se pričakovano število skupnih bitov enakih $1$ in minimum ter maksimum števila po naključju ujemajočih se bitov. Na podlagi teh števil se izračuna ocena podobnosti filtrov.
....

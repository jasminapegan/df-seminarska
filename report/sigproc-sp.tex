...
Najprej si oglejmo delovanje funkcije, ki jo predlagajo avtorji članka \cite{fbhash} in je bila prva opisana v poglavju \ref{utezivclanku}. Rezultati so predstavljeni na sliki \ref{fig:clanekUtezi}. V vseh nadaljnih grafih stolpci predstavljajo povprečno ujemanje, ki ga je algoritem zaznal glede na dejansko ujemanje med datotekama, črta pa ponazarja delež dokumentov s posameznim ujemanjem, pri katerem je algoritem ujemanje sploh prepoznal. Algoritem s to formulo za uteži je dosegel F-score 94\%, kar se ujema z vrednostjo, ki so jo dobili v članku.
\begin{figure}[htb]
\begin{center}
